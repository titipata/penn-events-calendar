...
def generate_location_candidate(event):
    ...
    location_candidate = [ent.text for ent in location.ents if ent.label_ in ('ORG', 'LOC')]
...
def generate_description_candidate(event):
    ...
    """
    event_text = event['speaker'] + ' ' + event['title'] + ' ' + event['description']
    candidates = [ent.text for ent in nlp(event_text).ents if ent.label_ in ('PERSON', 'ORG')]
...
if __name__ == '__main__':
    ...
    description_candidates = events_df.apply(generate_description_candidate, axis=1)
    ...
    suggest_candidates = (location_candidates + owner_candidates + description_candidates).\
        map(lambda x: list(pd.unique([e.strip() for e in x if e is not '' and len(e.split()) <= 4])[0:10]))
    events_df['suggest_candidates'] = suggest_candidates
    print('Done!')

    # save to JSON file
    save_json(events_df[['event_index', 'event_vector', 'suggest_candidates']].to_dict(orient='records'), PATH_VECTOR)
